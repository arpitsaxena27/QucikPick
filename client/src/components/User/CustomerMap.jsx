...
const CustomerMap = ({
      ...
      useEffect(() => {
            if (mapImage) {
                  ...
                  img.src = mapImage;
                  img.onload = () => {
                        console.log("Map image loaded:", img.width, img.height);
      ...
      useEffect(() => {
            ...
            const targetShelf = shelves.find(
                  (s) =>
                        s.nid?.toLowerCase().trim() ===
                        selectedProduct.nid?.toLowerCase().trim()
            );

            if (
                  !targetShelf ||
                  !targetShelf.boundingBox?.x ||
                  !targetShelf.boundingBox?.y ||
                  !targetShelf.boundingBox?.width ||
                  !targetShelf.boundingBox?.height
            ) {
                  console.warn(
                        "Shelf not found or missing boundingBox data for product:",
                        selectedProduct.nid
                  );
                  setPath([]);
                  setDestShelf(null);
            ...
            const dest = {
                  x: Math.floor(
                        (targetShelf.boundingBox.x +
                              targetShelf.boundingBox.width / 2) /
                              gridSize
                  ),
                  y: Math.floor(
                        (targetShelf.boundingBox.y +
                              targetShelf.boundingBox.height / 2) /
                              gridSize
                  ),
            };
                  ...
                  >
                        ...
                        >
                              ...
                              >
                                    ...
                                    {startLocation && (
                                          <g
                                                key={`ripples-${startLocation.x}-${startLocation.y}`}
                                          >
                                                <motion.circle
                                                      cx={
                                                            gridToPixel(
                                    ...
                                    {path.length > 1 && (
                                          <AnimatedPath
                                                points={path.map(gridToPixel)}
                                          />
                                    )}

                                    {/* Shelves */}
                                    {shelves.map((shelf) => {
                                          // Skip shelves that don't have boundingBox data
                                          if (
                                                !shelf.boundingBox?.x ||
                                                !shelf.boundingBox?.y ||
                                                !shelf.boundingBox?.width ||
                                                ...
                                                console.warn(
                                                      `Shelf ${shelf.nid} is missing boundingBox data`
                                                );
                                                return null;
                                          }

                                          const isDest =
                                                destShelf?.nid === shelf.nid;
                                          return isDest ? (
                                                <motion.rect
                                                      key={shelf.nid}
